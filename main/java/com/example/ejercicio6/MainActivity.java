...
ActivityResultLauncher<Intent> launcherfav;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adaptador adap = new adaptador(peliculas);
        ...
        launcherfav = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), new ActivityResultCallback<ActivityResult>() {
            @Override
            public void onActivityResult(ActivityResult result) {
                ArrayList<Boolean> fav = (ArrayList<Boolean>)result.getData().getSerializableExtra("fav");

                for (int i = 0; i < fav.size(); i++){
                    peliculas.get(i).setFavorita(fav.get(i));
                }
            }
        });
}
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            ...
            case R.id.favorito:
                ...
                it.putExtra("size",peliculas);
                launcherfav.launch(it);
                return true;
            case R.id.añadir:
                return true;
        }
